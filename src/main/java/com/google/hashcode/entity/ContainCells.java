 ...
 */
public class ContainCells {
    private static final Logger LOGGER = LoggerFactory.getLogger(ContainCells.class);
    ...
    public ContainCells(Cell... cell) {
        this.cells = new ArrayList<>(Arrays.asList(cell));
    }

    public ContainCells(List<Cell> cells) {
        this.cells = cells;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ContainCells)) return false;
        ContainCells slice = (ContainCells) o;
    ...
    public boolean isValid(ObjectLogic pizza) {
        ...
        int tomatoesNumber = this.cells.stream()
                ...
                .collect(Collectors.toList())
                .size();
        boolean isPassedSliceInstructions = this.cells.size() <= pizza.getInstruction().getMaxNumberOfCellsPerSlice()
                && tomatoesNumber >= pizza.getInstruction().getMinNumberOfIngredientPerSlice()
                && mushroomsNumber >= pizza.getInstruction().getMinNumberOfIngredientPerSlice();
        LOGGER.debug("\n" + pizza.getInstruction() +
                "\nContainCells :" + this +
    ...
    public Step generateStepAbove(ObjectLogic pizza) {
        ContainCells delta = new ContainCells();
    ...
    public Step generateStepBelow(ObjectLogic pizza) {
        ContainCells delta = new ContainCells();
        for (int x = this.minX(); x <= this.maxX(); x++) {
            //try to get a cell
    ...
    public Step generateStepLeft(ObjectLogic pizza) {
        ContainCells delta = new ContainCells();
    ...
    public Step generateStepRight(ObjectLogic pizza) {
        ContainCells delta = new ContainCells();
        for (int y = this.minY(); y <= this.maxY(); y++) {
            //try to get a cell
